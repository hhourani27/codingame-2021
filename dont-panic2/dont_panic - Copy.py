...
#Node
#(f,y,direction,round,elevators, add_elevators_left,action_for_next_round)
...
start_state = (start_floor,start_pos,RIGHT,1,nb_add_elevators)
...
while len(frontier) > 0:
    ...
    f,y,dir,round,add_elevator_left,action = current
    next_f,next_y,next_dir,next_round,next_add_elevator_left,next_action = current
    ...
    next_state = (next_f,next_y,next_dir,next_round,next_add_elevator_left)
...
#    if action != BLOCK and (next_f,next_y) not in elevators :
        next_BLOCK = next_state + (BLOCK,)
        frontier.append(next_BLOCK)
        parent[next_BLOCK] = current
    
    if next_add_elevator_left > 0 and next_f < exit_floor:
